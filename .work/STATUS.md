...
## Fase atual: BACKEND das Fases 4–6 em andamento. ✅ Sessão 1 — Financeiro. ✅ Sessão 2 — Estratégia + Profiles. ✅ Sessão 3 — NCT + Tarefas. Próximo: Sessão 4 — Dashboard.
...
4. **Backend das Fases 4–6** (em andamento): **Sessão 1 — Financeiro ✅** (migrations 0011–0013). **Sessão 2 — Estratégia + Profiles ✅** (queries/validations/action de strategy; profiles via prop). **Sessão 3 — NCT + Tarefas ✅** (queries/actions de nct + tasks-board; sem migration). **PRÓXIMO: Sessão 4 — Dashboard** (`docs/PROMPT.md`). Detalhe em `docs/PLANO-BACKEND-FINAL.md`.
...
Última sessão: **Sessão 3 — NCT + Tarefas (backend real)**. Sem migration (tabelas + FK `fk_tasks_commitment` já existiam). Novos: `lib/queries/nct.ts`, `lib/actions/nct.ts`, `lib/queries/tasks.ts`, `lib/actions/tasks-board.ts`. NCT/Tarefas (3 views + dialogs) religados às actions (otimista + resync); `projectLabels` via prop. `lib/mock/nct.ts`/`tasks.ts` ainda existem só p/ o Dashboard (Sessão 4). Build + 59 testes verdes. Próxima migration livre = `0014`. Gate validado pelo Octavio (narrativa→compromisso→check-in atualiza % na lista e no detalhe; board move/cria/filtra/persiste; tarefa vinculada aparece no detalhe).
