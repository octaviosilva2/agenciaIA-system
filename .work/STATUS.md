...
- **Tela de Manutenção com tarefas (`/manutencao/[contractId]`):** ✅ **UI front entregue (MOCK, sem backend)** — header do contrato, bloco **Cobrança editável** (valor mensal, duração, dia de cobrança, início, próximo contato, frequência, SLA, observações) e **kanban de tarefas** (mesmo `TasksKanban`, com **recorrência mensal** via flag `allowRecurrence`). Acessível pelo card do board `/manutencao` (passou a abrir a tela do contrato) **e** pelo atalho "Tarefas de manutenção" no bloco Manutenção da tela do projeto (`MaintenanceEditor`). Componentes: `components/projects/maintenance-detail.tsx` + página `app/(dashboard)/manutencao/[contractId]/page.tsx`. Tudo em estado local com toast; **nada persiste**. **Backend a cargo do sócio dev:** `tasks` ainda NÃO tem `contract_id` nem recorrência → exige **migration nova** (`contract_id`, e modelagem de recorrência — na UI ficou "todo mês no dia X", 1–28); `contracts` não tem método de pagamento padrão (método é por `charges.method`) — se quiser método no contrato, é coluna nova.
